...
namespace sc_m_c
{
	SCLaser::SCLaser()
	{	
		unsigned char start_port[2];
		start_port[0]=0xA5;
		start_port[1]=0xF0;						
		write_port(start_port,sizeof(start_port));
	}

	SCLaser::~SCLaser()
	{
		unsigned char end_port[2];
		printf("end port\n");
		end_port[0]=0xA5;
		end_port[1]=0xF5;
		write_port(end_port,sizeof(end_port));
		close(fd);
	}
...
#define RANGES_TIMP_SIZE 600 //tof雷达改为720
	...
	int SCLaser::poll(sensor_msgs::LaserScan::Ptr scan)
	{
		int Rcv=0,cpylen;
		int temp_depth;
		int i;
