...
class MultiRobotTF_Publisher:
    ...
    def __init__(self, method, weightpath):
        ...
        self.nn_output_dim = 256
        self.nn_input_dim = 512

        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        # device = torch.device("cpu")
        self.model = PNV.PointNetVlad(global_feat=True, feature_transform=True, max_pool=False,
                                output_dim=self.nn_output_dim, num_points=self.nn_input_dim)
    ...
    def listcallback(self, data):
        ...
        print("update submap list {}".format(current_robot_id))

        if not self.submap_list.has_key(current_robot_id):
            self.submap_list[current_robot_id] = SubmapList()
            self.feature_record[current_robot_id] = np.zeros((0,self.nn_output_dim),dtype=float)
    ...
    def findMatches(self, self_subamap, self_id, other_id):
        ...
        feature = self.map2feature(submap_result1.map, self.nn_input_dim)
        ...
        map = submap_result1.map.data
        map = np.array(map)
        map = map.reshape((submap_result1.map.info.height, submap_result1.map.info.width))
        cv2.imwrite("map_{}_{}.png".format(self_id, self_subamap.submap_index), map)
        ...
        if True:
            ...
            for i in range(len(results)):
                # if validmatch[i]:
                    ...
                    try:
                        ...
                        print(e)
                        confidence_result.append(0)
                        continue
                    confidence_result.append(tf_result.confidence)
            ...
            print("pointnet_results: "+ str(results))
            print("confidence_results: "+ str(confidence_result))
